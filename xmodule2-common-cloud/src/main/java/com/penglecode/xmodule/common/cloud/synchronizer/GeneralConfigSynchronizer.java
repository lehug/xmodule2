 ...
 */
public abstract class GeneralConfigSynchronizer<T extends GeneralConfigSyncRemoteBusEvent> implements SpringAppInitializer, ApplicationListener<T> {
	...
	public void onApplicationEvent(T event) {
		LOGGER.info("<<< 收到{}同步总线事件: [{} -> {}]", getConfigDescription(), event.getOriginService(), event.getDestinationService());
		updateConfig();
	}

...
public class OAuth2ExceptionResponseBodyHandler implements ResponseBodyAdvice<Object> {
	...
	public Object beforeBodyWrite(Object body,
		...
		if(body instanceof OAuth2Exception) {
			String tokenServiceRequestSource = request.getHeaders().getFirst(OAuth2ApplicationConstants.OAUTH2_TOKEN_SERVICE_REQUEST_SOURCE);
			if(!RemoteTokenServices.class.getSimpleName().equals(tokenServiceRequestSource)) {
				...
				response.setStatusCode(ObjectUtils.defaultIfNull(httpStatus, HttpStatus.INTERNAL_SERVER_ERROR));
